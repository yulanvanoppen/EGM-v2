...
%(adapted from https://github.com/yulanvanoppen/GMGTS)
%   
...
properties (Access = private)
    ...
    methods
        ...
        function traces = auxiliary(obj, parameters, init, t, tol)      % Numerically integrate auxiliary ODE system
            ...
            init_Phi = eye(obj.K);
            if string(obj.name) == "STAT5"
                init_Phi(1, [2 3]) = [1 2];
                init_Phi(2, 3) = 2;
            end
            init_Phi = reshape(init_Phi, 1, []);
        ...
        function process(obj, model_file, auxiliary_file)               % Process model files
            ...
            [~, obj.input, ~] = IQMfunctions(obj.model);
            if isempty(obj.input), obj.input = {'0'}; end
            ...
            [f, t, u, x, beta] = obj.construct_symbolic(f);
            obj.construct_linear_decomposition(f, t, u, x, beta);
            obj.construct_rhs_jacobian(f, t, u, x, beta);
        ...
        function [f, t, u, x, beta] = construct_symbolic(obj, f)        % Setup symbolic function
            obj.K = length(obj.states);
            t = sym('t', 'real');
            x = sym('x', [obj.K, 1], 'real');
            u = sym('u', 'real');
            f = replace(f, 'input(time)', 'input');
            f = subs(str2sym(f), [cellstr(obj.states') {'time', 'input'}], [x' t u]);
        ...
        function construct_linear_decomposition(obj, f, t, u, x, beta)     % Extract RHS decomposition g(x)⋅k+h(x)
            ...
            for l = 1:obj.K                                                 
                ...
                dg = matlabFunction(dg_vectorized, 'Vars', [x; t; u]);
                dg = @(states, times) dg(states{:}, times, obj.input(times));
                ...
                dh_symb = matlabFunction(dh_vectorized, 'Vars', [x; t; u]);
                dh_symb = @(states, times) dh_symb(states{:}, times, obj.input(times));
            ...
            h_symb = matlabFunction(permute(h_symb, [2 1]), 'Vars', [x; t; u]);% vectorize analogously
            h_symb = @(states, times) h_symb(states{:}, times, obj.input(times));
            obj.h_handle = @(states, times) h_symb(obj.deal_states(states), ...
                                                   obj.deal_times(times, size(states, 3)));
        end
        
        
        function construct_rhs_jacobian(obj, f, t, u, x, beta)             % RHS state derivatives
            ...
            for l = 1:obj.K
                df = matlabFunction(df_symb(l, :), 'Vars', [x; t; u; beta]);   % vectorized matlabFunction
                df = @(states, times, parameters) df(states{:}, times, obj.input(times), parameters{:});
