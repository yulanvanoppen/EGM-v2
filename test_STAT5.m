...
[SS_EGM, SS_TM] = deal(zeros(nseeds, 1));
[GSS_EGM, GSS_TM] = deal(zeros(nseeds, 1));
...
for seed = 1:nseeds
    ...
    estimator = EGM(system, data, Knots={[12 18 30 40], [6 10 14 18 25 40]}, ...    % different knots per state
                ...
                Prior=struct('mean', [1 1 1], 'prec', diag([0 0 0])));      % prior to prevent unbounded k2
    
    out1 = estimator.estimate(2*system.k0');                                % estimate using EGM
    out2 = estimator.estimate_TM(2*system.k0', [1 .1 .01 .001 0 0 0]+1E-8); % estimate using TM
    ...
    SS_EGM(seed) = sum_of_squares(out1.beta, data, ground_truth, system);
    SS_TM(seed) = sum_of_squares(out2.beta, data, ground_truth, system);
    GSS_EGM(seed) = GSS(out1.beta, data, system, estimator, out1.filtered);
    GSS_TM(seed) = GSS(out2.beta, data, system, estimator, out1.filtered);
end

save('simulation/test_STAT5_new2.mat')                                           % store results
